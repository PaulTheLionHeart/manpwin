...
CDib::CDib(const CDib & Dib1)	// Copy Constructor
    ...
    memcpy(DibPixels.data(), DIBPEL(pDibInf), SizeImage);
    }
...
//    BYTE *	SourcePixels = DIBPEL(pDib);
	...
	case 1:
	    ...
	    for(i = 0; i < height; ++i)
	    	{
		Dest = (RGBTRIPLE *)(DibPixels.data() + i * DestWidthBytes);
...
//		    }
	    ...
	    break;
	case 24:
	    memcpy(DibPixels.data(), SourcePixels, SizeImage);
	    break;
	case 32:
	    {
	    DWORD	SourcePel32;
...
//		    }
	...
	default:
	    ...
	    memcpy(DibPixels.data(), SourcePixels, SizeImage);	// just copy pixels
	    break;
	}
    }
...
BYTE	*CDib::InitDib(LPBITMAPINFO pDib)
    {
    CloseDibPtrs();			// give back memory for new DIB
...
BYTE	*CDib::InitDib(int Width, int Height)
    {
...
void CDib::ClearDib(int Red, int Green, int Blue)
	...
	case 24:
		...
		break;
	default:
	    memset((BYTE *)DibPixels.data(), 0, (size_t)SizeImage);	// if not 24 bit set to 0
	    break;
	}
    }
...
void	CDib::Text2Dib(HDC hDc, RECT *rect, COLORREF FontColour, COLORREF BkgColour, LOGFONT *lf, int TranparentText, TCHAR *text)
    ...
    if (SetDIBits(hDc, hBitmap, 0, DibHeight, DibPixels.data(), (LPBITMAPINFO)pDibInf, DIB_RGB_COLORS) == 0)
	return;
    DrawText(hdc, text, -1, rect, DT_CENTER);
    if (GetDIBits(hDc, hBitmap, 0, DibHeight, DibPixels.data(), (LPBITMAPINFO)pDibInf, DIB_RGB_COLORS) == 0)
	return;
