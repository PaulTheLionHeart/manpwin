...
void	CPerturbation::RefFunctions(BigComplex *centre, BigComplex *Z, int &SlopeDegree)
	...
	case 0:							// optimise for Mandelbrot by taking out as many steps as possible
	    ...
	    mpfr_mul(RealImag, ZTimes2.x, Z->y.x, MPFR_RNDN);
	...
	case 2:							// Burning Ship
	    ...
	    mpfr_sub(TempReal, SqrReal, SqrImag, MPFR_RNDN);
	    mpfr_add(Z->x.x, TempReal, centre->x.x, MPFR_RNDN);
	    mpfr_mul(TempImag, ZTimes2.x, Z->y.x, MPFR_RNDN);
...
void	CPerturbation::LoadPascal(std::vector<long> &PascalArray, int n)
    {
    long    j, c = 1L;
