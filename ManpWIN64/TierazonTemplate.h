...
void TierazonInit_Generic(
	...
	case 13:					// z=z*z*z*z*z*z+c
	    {
	    // degree remains external (legacy behaviour)
	    *degree = (int)param[0];
	    if (*degree < 1)
		*degree = 1;
	...
	case 102:
	    ...
	    // Always initialise z2 for safety
	    z2->x = 0.0;
	    z2->y = 0.0;
...
*/
	...
	case 106:					// More Fractals, 5th order N/Mset
		...
		*degree = 2;
		...
		z->x = -(q->x) * factor + param[1];
		z->y = -(q->y) * factor + param[2];
		}
	...
	case 136:					// Flarium 31, Polynomial: z = z*z*z-aa3*z+b
	    {
	    TComplex t = (q->CCube() - 1.0) / (q->CSqr() * 3.0);
	    TComplex a = t;
...
//	    *z = z->CCube() - z2.CCube() + *q;
	...
	case 165:					// Flarium 67-69, Newton Variations: z = ((z-(((z^n)-1)/(n*(z^(n-1)))))^2)*c
	    {
	    TComplex z_old = *z;
	    TComplex f1n = z->CPolynomial(*degree - 1);
	    TComplex fn = f1n * *z;

	    *z = *z - (fn - 1.0) / (f1n * (*degree));
	    *z = z->CSqr() * *q;
	    zd = *z - z_old;
	    return DoBailout(bailout_type, &zd, rqlim);
	    }
	...
	case 166:					// Flarium 80, Newton Variations: z=((z*z*z*z-1)/(1-(3*z*z*z)))*c
	    z1 = *z;
	    z3 = z->CCube();	// A correct one, finally
	    *z = ((*z * z3 - 1.0) / (TComplex(1.0) - z3 * 3.0)) * *q;
	    zd = *z - z1;
	    d = zd.CSumSqr();
	    return (d < MINSIZE);
