...
int	DoSprott374(void)
	...
	if (DisplayOscillator(c1, cn, dt, ((DWORD)(i / 60.0) % gManp->threshold), i, 3, 0) < 0)
	    break;
	}
    PlotExtras();
    return 0;
...
int	DoSprott421(void)
    ...
    c1[0] = gManp->param[10];	// q
    c1[1] = gManp->param[11];	// p
    c1[2] = gManp->param[12];	// z
...
int	DoSprottLinzA(void)
	...
	cn[1] = -c1[0] + c1[1] * c1[2];
	cn[2] = 1 - c1[1] * c1[1];
	if (DisplayOscillator(c1, cn, dt, ((DWORD)i % gManp->threshold), i, 3, 0) < 0)
	    break;
	}
    PlotExtras();
    return 0;
...
int	DoSprottLinzQ(void)
	...
	if (DisplayOscillator(c1, cn, dt, ((DWORD)(i * 0.01) % gManp->threshold), i, 3, 0) < 0)
	    break;
	}
    PlotExtras();
    return 0;
...
int	DoSprottMarcus(void)
	...
	if (DisplayOscillator(c1, cn, dt, ((DWORD)(i / 5.0) % gManp->threshold), i, 3, 0) < 0)
	    break;
	}
    PlotExtras();
    return 0;
...
int	DoSprottWind(void)
	...
	if (DisplayOscillator(c1, cn, dt, ((DWORD)(i / 35.0) % gManp->threshold), i, 3, 0) < 0)
	    break;
	}
    PlotExtras();
    return 0;
