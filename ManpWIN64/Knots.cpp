...
int	DoAnIntroductions2HarmonicKnots(void)
	...
	if (user_data(gManp->GlobalHwnd) == -1)				// user pressed a key?
	    return -1;
	if (DisplayKnot(c1, ((DWORD)(i / ColourFactor) % gManp->threshold), (int)KnotWidth, i, 6) < 0)
	    break;
	t += gap;
	}
    PlotExtras();
...
int	DoBorromeanRings(void)
	...
	for (j = 0; j < 3; j++)
	    c1[j + 3] += c1[j] * dt;
	if (DisplayKnot(c1, ((DWORD)(i * 2 / ColourFactor) % gManp->threshold), (int)KnotWidth, i, 6) < 0)
	    break;
	...
	for (j = 0; j < 3; j++)
	    c1[j + 3] += c1[j] * dt;
	if (DisplayKnot(c1, ((DWORD)(i * 3 / ColourFactor) % gManp->threshold), (int)KnotWidth, i, 6) < 0)
	    break;
	t += gap;
	}
    PlotExtras();
...
int	DoKnotUniversesI(void)
    ...
    c1[0] = c1[4] = gManp->param[10];	// x
    c1[1] = c1[5] = gManp->param[11];	// y
    c1[2] = c1[6] = gManp->param[12];	// z
    c1[3] = c1[7] = gManp->param[12];	// z
	...
	if (user_data(gManp->GlobalHwnd) == -1)				// user pressed a key?
	    return -1;
	if (DisplayKnot(c1, ((DWORD)(i / ColourFactor) % gManp->threshold), (int)KnotWidth, i, 8) < 0)
	    break;
	t += gap;
	}
    PlotExtras();
...
int	DoLissajous(void)
    ...
    l1 = gManp->param[10]; 		// use variable parameters for additional constants
    l2 = gManp->param[11]; 
    l3 = gManp->param[12]; 
    ...
    k2 = gManp->param[6];
    k3 = gManp->param[7];
    KnotWidth = gManp->param[8];
    ColourFactor = gManp->param[9];
    gManp->totpasses = 10;
...
int	DoMarcusTorusKnot(void)
	...
	if (DisplayKnot(c1, ((DWORD)(i / ColourFactor) % gManp->threshold), (int)KnotWidth, i, 6) < 0)
//	if (DisplayOscillator(c1, cn, dt, ((DWORD)(t * ColourFactor) % gManp->threshold), t, 3, KnotWidth) < 0)
	    break;
	t += gap;
	}
    PlotExtras();
...
void	InitKnot(double c1[], int dimensions)
    {
    gManp->OscProcess.InitOscProc(dimensions, gManp->mandel_width, gManp->AspectRatio, gManp->xdots, gManp->ydots, gManp->type, gManp->subtype, gManp->OscAnimProc, &gManp->xAxis, &gManp->yAxis, &gManp->zAxis, BOUNDARY, &gManp->Dib, c1, gManp->PerspectiveFlag, gManp->iterations,
		gManp->OscBackGround, &gManp->x_rot, &gManp->y_rot, &gManp->z_rot, &RemoveHiddenPixels, gManp->GlobalHwnd, &gManp->TrueCol);
    }
...
int	DisplayKnot(double c1[], DWORD colour, int KnotWidth, double i, int dimensions)
    {
    return gManp->OscProcess.DisplayOscillator(c1, c1, 0.0, colour, i, dimensions, FALSE, 1, 1, gManp->hor, gManp->vert, VertBias, zBias, gManp->OscAnimProc, gManp->CoordSystem, gManp->threshold, 0.0, KnotWidth);
    }
...
int	DoKnots(void) 
    {
    return (KnotSpecific[gManp->subtype].calctype());
    }
