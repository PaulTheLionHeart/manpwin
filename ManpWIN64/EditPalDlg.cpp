...
INT_PTR CALLBACK EditPalDlg (HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
     {
     static int	    color [8] ;
     static int	    StartIter = 0;
     static int	    EndIter = 0;
     static std::vector<RGBTRIPLE>TempPal{ 0 };
     ...
     switch (message)
	  {
	  case WM_INITDIALOG:
		TempPal.resize(MAXPALETTE);				// for maximum palette
...
//			HaveValidRule = TRUE;
			EditPal.RedStart = TempPal[StartIter].rgbtRed & 0x00FF;
			EditPal.GreenStart = TempPal[StartIter].rgbtGreen & 0x00FF;
			EditPal.BlueStart = TempPal[StartIter].rgbtGreen & 0x00FF;
			EditPal.RedEnd = TempPal[EndIter].rgbtRed & 0x00FF;
			EditPal.GreenEnd = TempPal[EndIter].rgbtGreen & 0x00FF;
			EditPal.BlueEnd = TempPal[EndIter].rgbtBlue & 0x00FF;
			EditPal.ChangePixel(color, StartIter, EndIter, TempPal);
