/*
    FilterTemplate.h - Template for handling pixel functions for all arithmetic types.
...
long DoOutsideFilterT(long iteration, TComplex& z, int method, int hooper, long threshold, int paletteColours, int decomp,
    int biomorph, double rqlim, int special, bool logval, BYTE* logtable, int potentialColours, CTrueCol *TrueCol, double* potparam)
    {
    double magnitude = 0.0;
    CPotential Pot;

    if (paletteColours == 256 && decomp > 0)
	{
	iteration = DecompositionT<TComplex, TReal>(z.x, z.y, decomp, paletteColours);
	}
    else if (logval)
	{
	...
	switch (method)
	    ...
	    case POTENTIAL:
		magnitude = z.CSumSqr();
		iteration = Pot.potential(magnitude, iteration, threshold, TrueCol, potentialColours, potparam);
		break;

	    case PERT1:
		if (iteration != threshold)
		    iteration = (int)((iteration - log2(log2((double)z.CSumSqr()))) * 5) % 256;
		break;

	    case PERT2:
		if (iteration != threshold)
		    iteration = (int)(iteration - (log(0.5 * (double)z.CSumSqr()) - log(0.5 * log(256))) / log(2)) % 256;
		break;
	    }
...
long DoInsideFilterT(long iteration, TComplex& z, int InsideMethod, long threshold, double min_orbit, long min_index)
    {
    switch (InsideMethod)
	{
	case BOF60:
	    iteration = (long)(sqrt(min_orbit) * 75.0);
	    break;
	...
	case ZMAG:
	    iteration = (long)(ToDouble(z.CSumSqr()) * (threshold >> 1) + 1);
	    break;
