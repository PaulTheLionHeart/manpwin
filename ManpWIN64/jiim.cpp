...
unsigned short memory_handle = 0;
FILE *file;
...
static	WORD	dstack[MAXHORIZONTAL * 3];
...
extern	void	output_line(WORD, WORD, WORD, WORD *);
...
static void clip_put_line(int row, int start, int stop, WORD *pixels)
   {
   ...
   output_line(row, start, stop, pixels);
   }

void putrow(int x, int y, int width, WORD *buff)
   {
   clip_put_line(y, x, x+width-1, (WORD *)buff);
   }
...
//   memset(dstack, color % colors, width);
   memset(dstack, color & 0xff, width);
   while (depth-- > 0)
   {
	if (user_data(PixelHwnd) < 0)
//      if(keypressed()) /* we could do this less often when in fast modes */
         return;
      putrow(x, y++, width, (WORD *)dstack);
   }
}
