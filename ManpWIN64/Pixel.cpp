...
long	CPixel::dofract(HWND hwnd, int row, int col)
    ...
    // Fractal initialisation routines
    result = InitialisePixelFractal(tantable);

    if (result == PIXEL_BLUE)
	return BLUE;
    else if (result == PIXEL_ZERO)
	return 0L;

    // Runtime Initialisation
    InitialisePixelRuntime(saved, DDSaved, QDSaved, BigSaved, hooper, close, magnitude, min_orbit, min_index);

    for EVER
	{
	SaveOrbitHistory();				// for Forward differencing and Smoothing processing
	...
	if (!BigNumFlag && distest)			// --- distance estimator path (no point implementing anywhere except double)
	    result = RunDistanceEstimatorIteration();
	else						// the usual case
	    result = RunStandardIteration();		// arithmetic-specific fractal step

	if (result == PIXEL_BLUE)
	    return BLUE;
	else if (result == PIXEL_STOP)
	    break;
    ...
    if (FinaliseOrbitFeatures(iteration, tantable))
	return iteration;

    if (FinaliseDistanceEstimation(iteration))
	return iteration;

    iteration = ApplyFinalColour(iteration, hooper, min_orbit, min_index);
    ...
    StoreFloatIteration(row, col, iteration);
    return(iteration);
    }
...
void	CPixel::InitialisePixelRuntime(Complex& saved, DDComplex& DDSaved, QDComplex& QDSaved, BigComplex& BigSaved, int& hooper, double& close, double& magnitude, double& min_orbit, long& min_index)
    {
    // --- iteration state ---
    iteration = 0;
    real_iteration = 0;
...
/************************************************************************
    Initialisation routines for most fractals
************************************************************************/

int CPixel::InitialisePixelFractal(double (&tantable)[16])
    {
    if (!BigNumFlag)
	{
	...
	{
	if (cBig.x.BigDouble2DD(&cDD.x) < 0)
	    return PIXEL_ZERO;
	if (cBig.y.BigDouble2DD(&cDD.y) < 0)
	    return PIXEL_ZERO;
	...
	{
	if (cBig.x.BigDouble2QD(&cQD.x) < 0)
	    return PIXEL_ZERO;
	if (cBig.y.BigDouble2QD(&cQD.y) < 0)
	    return PIXEL_ZERO;
...
/************************************************************************
    Normal fractal orbit calculation

    return -1 = division by zero -> caller returns BLUE
    return  0 = continue orbit
    return  1 = stop orbit
************************************************************************/

int CPixel::RunStandardIteration()
    {
    switch (MathType)
	{
	case DOUBLEFLOAT:
	    return RunFractal(&z, &q);

	case DOUBLEDOUBLE:
	    return DDRunFractal(&zDD, &qDD);

	case QUADDOUBLE:
	    return QDRunFractal(&zQD, &qQD);

	case ARBITRARYPREC:
	    return BigRunFractal();
	}
...
/************************************************************************
    ...
    PIXEL_BLUE       = division by zero -> caller returns BLUE
    PIXEL_CONTINUE   = continue orbit
    PIXEL_STOP       = stop orbit
************************************************************************/

int CPixel::RunDistanceEstimatorIteration()
    {
    double ftemp;
    int result;
    ...
    if (result == PIXEL_BLUE)				// divide by zero error
	return PIXEL_BLUE;				// division by zero (Was Blue)

    else if (result)
	{
	if (dem_color < 0)				// note "regular" colour for later
	    {
	    dem_color = iteration;
	    dem_new = z;
	    }
...
/************************************************************************
    Save orbit history (used in Forward differencing and Smoothing processing)
************************************************************************/

void CPixel::SaveOrbitHistory()
    {
    if (calcmode == 'F')
	{
	switch (MathType)
...
bool	CPixel::FinaliseDistanceEstimation(long &iteration)
    {
	...
	if (iteration >= threshold)
	    {
	    iteration = threshold;
	    return true;						// original dofract() returned here
	    }
...
/************************************************************************
    Tierazon finalisation, StarTrail and Special colour processing
************************************************************************/

bool CPixel::FinaliseOrbitFeatures(long& iteration, double* tantable)
    {
    Complex	tempComplex;
...
/************************************************************************
    Forward differencing and Smoothing processing
************************************************************************/

void CPixel::StoreFloatIteration(int row, int col, long ColourIndex)
    {
	...
	if (col >= 0 && col < xdots - 1 && row >= 0 && row < ydots - 1)
		...
		gManp->PixelFlags[index] |= PIXEL_SPECIAL;
		}
	    else if ((long)FloatIteration >= threshold)
		{
		// Inside filters have already selected the colour index.
		// Preserve it for the Forward Difference renderer.
		gManp->wpixels[index] = (float)ColourIndex;
		gManp->PixelFlags[index] |= PIXEL_INSIDE;
		}
		...
		{
		// Exterior pixels keep their fractional iteration height.
		gManp->wpixels[index] = (float)FloatIteration;
		}
	    }
	}
...
bool	CPixel::ProcessOrbitFeatures(int& hooper, double close, double& magnitude, double& min_orbit, long& min_index, double* tantable)
    {
    DDComplex	DDTemp;
    QDComplex	QDTemp;
    Complex	tempComplex;
...
void CPixel::CheckPeriodicity(Complex& saved, DDComplex& DDSaved, QDComplex& QDSaved, BigComplex& BigSaved)
    {
    switch (MathType)
	{
	case DOUBLEFLOAT:
...
long CPixel::ApplyFinalColour(long iteration, int hooper, double min_orbit, long min_index)
    {
    Complex	tempComplex;
	...
	switch (MathType)
	    {
	    case DOUBLEFLOAT:
		iteration = DoOutsideFilterT<Complex, double>(iteration, z, OutsideMethod, hooper, threshold, paletteColours, decomp, biomorph,
		    rqlim, special, logval, logtable, potentialColours, TrueCol, potparam);
		break;
		...
		iteration = DoOutsideFilterT<Complex, double>(iteration, tempComplex, OutsideMethod, hooper, threshold, paletteColours, decomp, biomorph,
		    rqlim, special, logval, logtable, potentialColours, TrueCol, potparam);
		break;
	...
	switch (MathType)
	    {
	    case DOUBLEFLOAT:
		iteration = DoInsideFilterT(iteration, z, InsideMethod, threshold, min_orbit, min_index);
		break;

	    case DOUBLEDOUBLE:
		iteration = DoInsideFilterT(iteration, zDD, InsideMethod, threshold, min_orbit, min_index);
		break;

	    case QUADDOUBLE:
		iteration = DoInsideFilterT(iteration, zQD, InsideMethod, threshold, min_orbit, min_index);
		break;

	    case ARBITRARYPREC:
		iteration = DoInsideFilterT(iteration, zBig, InsideMethod, threshold, min_orbit, min_index);
		break;
	    }
	}
    return iteration;
...
long	CPixel::calc_frac(HWND hwnd, int row, int col, int user_data(HWND hwnd))
    ...
    if (calcmode == 'B')
	{
	if (color >= paletteColours)			// don't use color 0 unless from inside
	    if (paletteColours < 16)
		color &= andcolor;
	    else
