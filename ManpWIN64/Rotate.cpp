...
//static	RGBTRIPLE	temp_palette[MAXPALETTE];
...
//    InitTrueColourPalette(FALSE);
	...
	for (m = 0L; m < Dib.DibWidth; ++m)
	    ...
	    pointer = m + m + m + i;
	    Dib.DibPixels[pointer + 0] = TrueCol.PalettePtr[n].rgbtRed;
	    Dib.DibPixels[pointer + 1] = TrueCol.PalettePtr[n].rgbtGreen;
	    Dib.DibPixels[pointer + 2] = TrueCol.PalettePtr[n].rgbtBlue;
	    }
//	for (n = 0; n < 3; ++n)
