...
int	CPreview::PreviewGrid(HWND hwnd, int type)
	...
	for (i = 0; i < PreviewDib.DibHeight; ++i)
		...
		*(PreviewDib.DibPixels.data() + address + k) = ~(*(PreviewDib.DibPixels.data() + address + k));
	    }
	}
	...
	for (i = 0; i < PreviewDib.DibWidth; ++i)
	    ...
	    j = bytes_per_pixel * i;
	    for (k = 0; k < bytes_per_pixel; ++k)
		*(PreviewDib.DibPixels.data() + address + (long)k + j) = ~(*(PreviewDib.DibPixels.data() + address + (long)k + j));
	    }
	}
    if (type == GRIDLINES)
	{
...
int	CPreview::CopyPreview(HWND hwnd, CDib *Dib)
	...
	{
	tmp = (RGBTRIPLE *)(Dib->DibPixels.data() + (Dib->WidthBytes * ((DWORD)i * (DWORD)Dib->DibHeight / (DWORD)PreviewHeight)));
	Dest = (RGBTRIPLE *)(PreviewDib.DibPixels.data() + PreviewDib.WidthBytes * (DWORD)i);
...
int	CPreview::Preview(HWND hwnd)
    ...
    StretchDIBits (hdc, HorOffset, VertOffset, 
		    ...
		    (LPSTR)PreviewDib.DibPixels.data(), (LPBITMAPINFO)PreviewDib.pDibInf, DIB_PAL_COLORS, SRCCOPY);
    ShowWindow (hwnd, SW_SHOWNORMAL);
