/*
    LINE3D.CPP - 3D projection
    
    This version reads values of count from file and displays the data transformed into 
    a 3 dimensional image. For more information refer to Scientific American August 1985.
...
void	CPixel::projection(int x, int y, long col)
    ...
    if (x < 0 || x >= (int)oldline.size())
	return;
    colour = (col >= MAXPALETTE) ? MAXPALETTE : (int)col;	// reduce to 15 bits
    cur.colour = colour;
