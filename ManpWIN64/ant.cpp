...
int	COtherFunctions::tower(void)
    {
    std::vector<double>  z{ 0 };	// z is a 2D matrix that is the number of hits in the coordinates 'c' and 'd' that I use.
    std::vector<double>  m{ 0 };	// m is the height of the number of hits. 'c' and 'd' are respectively the abscissae and co-ordinates.
    std::vector<double>  l{ 0 };	// co-ordinates
    std::vector<double>  p{ 0 };	// co-ordinates
...
//    int RandomDigit = rand() % 10 - 5;
    c = a + 1;
    d = b - 2;
    z.resize(MAXIMUM_TOWER * MAXIMUM_TOWER);
    m.resize(MAXIMUM_TOWER);
    l.resize(MAXIMUM_TOWER);
	...
	BGColPtr = 255;
    int		red, green, blue;
    red = TrueCol->DefaultPalettePtr[BGColPtr].rgbtRed;
    green = TrueCol->DefaultPalettePtr[BGColPtr].rgbtGreen;
    blue = TrueCol->DefaultPalettePtr[BGColPtr].rgbtBlue;
    BackGround = blue + (green << 8) + (red << 16);
