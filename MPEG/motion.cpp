...

{
  ...
  mb = cur + i + FrameWidth*j;

  var = variance(mb,FrameWidth);
  ...
  else if (pict_type==P_TYPE)
  {
    if (frame_pred_dct)
    {
      dmc = fullsearch(oldorg,oldref,mb,
                       FrameWidth,i,j,sxf,syf,16,FrameWidth,FrameHeight,&imin,&jmin);
      vmc = dist2(oldref+(imin>>1)+FrameWidth*(jmin>>1),mb,
                  FrameWidth,imin&1,jmin&1,16);
    ...
    else
    {
      ...
      else
      {
        ...
        dmc = dmcfield;
        vmc = dist2(oldref+(tsel?FrameWidth:0)+(imint>>1)+(FrameWidth<<1)*(jmint>>1),
                    mb,FrameWidth<<1,imint&1,jmint&1,8);
        vmc+= dist2(oldref+(bsel?FrameWidth:0)+(iminb>>1)+(FrameWidth<<1)*(jminb>>1),
                    mb+FrameWidth,FrameWidth<<1,iminb&1,jminb&1,8);
      }
    }
    ...
    else
    {
       ...
       */
      v0 = dist2(oldref+i+FrameWidth*j,mb,FrameWidth,0,0,16);
  ...
  else /* if (pict_type==B_TYPE) */
  {
    if (frame_pred_dct)
    {
      ...
      dmcf = fullsearch(oldorg,oldref,mb,
                        FrameWidth,i,j,sxf,syf,16,FrameWidth,FrameHeight,&iminf,&jminf);
      vmcf = dist2(oldref+(iminf>>1)+FrameWidth*(jminf>>1),mb,
                   FrameWidth,iminf&1,jminf&1,16);

      /* backward */
      dmcr = fullsearch(neworg,newref,mb,
                        FrameWidth,i,j,sxb,syb,16,FrameWidth,FrameHeight,&iminr,&jminr);
      vmcr = dist2(newref+(iminr>>1)+FrameWidth*(jminr>>1),mb,
                   FrameWidth,iminr&1,jminr&1,16);

      /* interpolated (bidirectional) */
      vmci = bdist2(oldref+(iminf>>1)+FrameWidth*(jminf>>1),
                    newref+(iminr>>1)+FrameWidth*(jminr>>1),
                    mb,FrameWidth,iminf&1,jminf&1,iminr&1,jminr&1,16);
    ...
    else
    {
      ...
      /* frame */
      dmci = bdist1(oldref+(iminf>>1)+FrameWidth*(jminf>>1),
                    newref+(iminr>>1)+FrameWidth*(jminr>>1),
                    mb,FrameWidth,iminf&1,jminf&1,iminr&1,jminr&1,16);

      /* top field */
      dmcfieldi = bdist1(
                    oldref+(imintf>>1)+(tself?FrameWidth:0)+(FrameWidth<<1)*(jmintf>>1),
                    newref+(imintr>>1)+(tselr?FrameWidth:0)+(FrameWidth<<1)*(jmintr>>1),
                    mb,FrameWidth<<1,imintf&1,jmintf&1,imintr&1,jmintr&1,8);

      /* bottom field */
      dmcfieldi+= bdist1(
                    oldref+(iminbf>>1)+(bself?FrameWidth:0)+(FrameWidth<<1)*(jminbf>>1),
                    newref+(iminbr>>1)+(bselr?FrameWidth:0)+(FrameWidth<<1)*(jminbr>>1),
                    mb+FrameWidth,FrameWidth<<1,iminbf&1,jminbf&1,iminbr&1,jminbr&1,8);
          ...
          && dmci<dmcr && dmci<dmcfieldr)
      {
        ...
        mbi->mb_type = MB_FORWARD|MB_BACKWARD;
        mbi->motion_type = MC_FRAME;
        vmc = bdist2(oldref+(iminf>>1)+FrameWidth*(jminf>>1),
                     newref+(iminr>>1)+FrameWidth*(jminr>>1),
                     mb,FrameWidth,iminf&1,jminf&1,iminr&1,jminr&1,16);
      }
               ...
               && dmcfieldi<dmcr && dmcfieldi<dmcfieldr)
      {
        ...
        mbi->mb_type = MB_FORWARD|MB_BACKWARD;
        mbi->motion_type = MC_FIELD;
        vmc = bdist2(oldref+(imintf>>1)+(tself?FrameWidth:0)+(FrameWidth<<1)*(jmintf>>1),
                     newref+(imintr>>1)+(tselr?FrameWidth:0)+(FrameWidth<<1)*(jmintr>>1),
                     mb,FrameWidth<<1,imintf&1,jmintf&1,imintr&1,jmintr&1,8);
        vmc+= bdist2(oldref+(iminbf>>1)+(bself?FrameWidth:0)+(FrameWidth<<1)*(jminbf>>1),
                     newref+(iminbr>>1)+(bselr?FrameWidth:0)+(FrameWidth<<1)*(jminbr>>1),
                     mb+FrameWidth,FrameWidth<<1,iminbf&1,jminbf&1,iminbr&1,jminbr&1,8);
      }
      else if (dmcf<dmcfieldf && dmcf<dmcr && dmcf<dmcfieldr)
      {
        ...
        mbi->mb_type = MB_FORWARD;
        mbi->motion_type = MC_FRAME;
        vmc = dist2(oldref+(iminf>>1)+FrameWidth*(jminf>>1),mb,
                    FrameWidth,iminf&1,jminf&1,16);
      }
      else if (dmcfieldf<dmcr && dmcfieldf<dmcfieldr)
      {
        ...
        mbi->mb_type = MB_FORWARD;
        mbi->motion_type = MC_FIELD;
        vmc = dist2(oldref+(tself?FrameWidth:0)+(imintf>>1)+(FrameWidth<<1)*(jmintf>>1),
                    mb,FrameWidth<<1,imintf&1,jmintf&1,8);
        vmc+= dist2(oldref+(bself?FrameWidth:0)+(iminbf>>1)+(FrameWidth<<1)*(jminbf>>1),
                    mb+FrameWidth,FrameWidth<<1,iminbf&1,jminbf&1,8);
      }
      else if (dmcr<dmcfieldr)
      {
        ...
        mbi->mb_type = MB_BACKWARD;
        mbi->motion_type = MC_FRAME;
        vmc = dist2(newref+(iminr>>1)+FrameWidth*(jminr>>1),mb,
                    FrameWidth,iminr&1,jminr&1,16);
      }
      else
      {
        ...
        mbi->mb_type = MB_BACKWARD;
        mbi->motion_type = MC_FIELD;
        vmc = dist2(newref+(tselr?FrameWidth:0)+(imintr>>1)+(FrameWidth<<1)*(jmintr>>1),
                    mb,FrameWidth<<1,imintr&1,jmintr&1,8);
        vmc+= dist2(newref+(bselr?FrameWidth:0)+(iminbr>>1)+(FrameWidth<<1)*(jminbr>>1),
                    mb+FrameWidth,FrameWidth<<1,iminbr&1,jminbr&1,8);
      }
    }
...
static void field_ME(unsigned char *oldorg,unsigned char *neworg,unsigned char *oldref,
			unsigned char *newref,unsigned char *cur,unsigned char *curref,int i,int j,
			int sxf,int syf,int sxb,int syb,struct mbinfo *mbi,int secondfield,int ipflag)

{
  ...
  else if (pict_type==P_TYPE)
  {
    toporg = oldorg;
    topref = oldref;
    botorg = oldorg + FrameWidth;
    botref = oldref + FrameWidth;
    ...
    if (secondfield)
    {
      ...
      if (pict_struct==TOP_FIELD)
      {
        ...
        botorg = cur + FrameWidth;
        botref = curref + FrameWidth;
      }
      else
      {
  ...
  {
    /* forward prediction */
    field_estimate(oldorg,oldref,oldorg+FrameWidth,oldref+FrameWidth,mb,
                   i,j,sxf,syf,0,
                   &iminf,&jminf,&imin8uf,&jmin8uf,&imin8lf,&jmin8lf,
                   &dmcfieldf,&dmc8f,&self,&sel8uf,&sel8lf,&imins,&jmins,&ds);

    /* backward prediction */
    field_estimate(neworg,newref,neworg+FrameWidth,newref+FrameWidth,mb,
                   i,j,sxb,syb,0,
    ...
    /* field */
    dmcfieldi = bdist1(oldref + (self?FrameWidth:0) + (iminf>>1) + w2*(jminf>>1),
                       newref + (selr?FrameWidth:0) + (iminr>>1) + w2*(jminr>>1),
                       mb,w2,iminf&1,jminf&1,iminr&1,jminr&1,16);

    /* 16x8 upper half block */
    dmc8i = bdist1(oldref + (sel8uf?FrameWidth:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
                   newref + (sel8ur?FrameWidth:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
                   mb,w2,imin8uf&1,jmin8uf&1,imin8ur&1,jmin8ur&1,8);

    /* 16x8 lower half block */
    dmc8i+= bdist1(oldref + (sel8lf?FrameWidth:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
                   newref + (sel8lr?FrameWidth:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
        ...
        && dmcfieldi<dmcfieldr && dmcfieldi<dmc8r)
    {
      ...
      mbi->mb_type = MB_FORWARD|MB_BACKWARD;
      mbi->motion_type = MC_FIELD;
      vmc = bdist2(oldref + (self?FrameWidth:0) + (iminf>>1) + w2*(jminf>>1),
                   newref + (selr?FrameWidth:0) + (iminr>>1) + w2*(jminr>>1),
                   mb,w2,iminf&1,jminf&1,iminr&1,jminr&1,16);
    }
             ...
             && dmc8i<dmcfieldr && dmc8i<dmc8r)
    {
      ...
      /* upper half block */
      vmc = bdist2(oldref + (sel8uf?FrameWidth:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
                   newref + (sel8ur?FrameWidth:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
                   mb,w2,imin8uf&1,jmin8uf&1,imin8ur&1,jmin8ur&1,8);

      /* lower half block */
      vmc+= bdist2(oldref + (sel8lf?FrameWidth:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
                   newref + (sel8lr?FrameWidth:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
                   mb+8*w2,w2,imin8lf&1,jmin8lf&1,imin8lr&1,jmin8lr&1,8);
    }
    else if (dmcfieldf<dmc8f && dmcfieldf<dmcfieldr && dmcfieldf<dmc8r)
    {
      ...
      mbi->mb_type = MB_FORWARD;
      mbi->motion_type = MC_FIELD;
      vmc = dist2(oldref + (self?FrameWidth:0) + (iminf>>1) + w2*(jminf>>1),
    ...
    else if (dmc8f<dmcfieldr && dmc8f<dmc8r)
    {
      ...
      /* upper half block */
      vmc = dist2(oldref + (sel8uf?FrameWidth:0) + (imin8uf>>1) + w2*(jmin8uf>>1),
                  mb,w2,imin8uf&1,jmin8uf&1,8);

      /* lower half block */
      vmc+= dist2(oldref + (sel8lf?FrameWidth:0) + (imin8lf>>1) + w2*(jmin8lf>>1),
    ...
    else if (dmcfieldr<dmc8r)
    {
      ...
      mbi->mb_type = MB_BACKWARD;
      mbi->motion_type = MC_FIELD;
      vmc = dist2(newref + (selr?FrameWidth:0) + (iminr>>1) + w2*(jminr>>1),
    ...
    else
    {
      ...
      /* upper half block */
      vmc = dist2(newref + (sel8ur?FrameWidth:0) + (imin8ur>>1) + w2*(jmin8ur>>1),
                  mb,w2,imin8ur&1,jmin8ur&1,8);

      /* lower half block */
      vmc+= dist2(newref + (sel8lr?FrameWidth:0) + (imin8lr>>1) + w2*(jmin8lr>>1),
...
//int imins[2][2],jmins[2][2];
{
  ...
  /* frame prediction */
  *dframep = fullsearch(org,ref,mb,FrameWidth,i,j,sx,sy,16,FrameWidth,FrameHeight,
                        iminp,jminp);

  /* predict top field from top field */
  dt = fullsearch(org,ref,mb,FrameWidth<<1,i,j>>1,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                  &imint,&jmint);

  /* predict top field from bottom field */
  db = fullsearch(org+FrameWidth,ref+FrameWidth,mb,FrameWidth<<1,i,j>>1,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                  &iminb,&jminb);
  ...
  dt = fullsearch(org,ref,mb+FrameWidth,FrameWidth<<1,i,j>>1,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                  &imint,&jmint);

  /* predict bottom field from bottom field */
  db = fullsearch(org+FrameWidth,ref+FrameWidth,mb+FrameWidth,FrameWidth<<1,i,j>>1,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                  &iminb,&jminb);
...

{
  ...
  iminb = jminb = 0;	// PHD 2012/04/26. variables uninitialised????????
  ...
  if (notop)
    ...
    dt = fullsearch(toporg,topref,mb,FrameWidth<<1,
                    i,j,sx,sy>>1,16,FrameWidth,FrameHeight>>1,
                    &imint,&jmint);
  ...
  if (nobot)
    ...
    db = fullsearch(botorg,botref,mb,FrameWidth<<1,
                    i,j,sx,sy>>1,16,FrameWidth,FrameHeight>>1,
                    &iminb,&jminb);
  ...
  if (notop)
    ...
    dt = fullsearch(toporg,topref,mb,FrameWidth<<1,
                    i,j,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                    &imint,&jmint);
  ...
  if (notop)
    ...
    dt = fullsearch(toporg,topref,mb+(FrameWidth<<4),FrameWidth<<1,
                    i,j+8,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                    &imint,&jmint);
  ...
  if (nobot)
    ...
    db = fullsearch(botorg,botref,mb+(FrameWidth<<4),FrameWidth<<1,
                    i,j+8,sx,sy>>1,8,FrameWidth,FrameHeight>>1,
                    &iminb,&jminb);
...
//int iminf[2][2], jminf[2][2];
{
  ...
  imins = jmins = imint = jmint = iminb = jminb = imindmv = jmindmv = 0;    // silly uninitialsed warnings?
  ...
  for (pref=0; pref<2; pref++)
  {
    for (ppred=0; ppred<2; ppred++)
    {
      ...
      if (is >= 0 && is <= (FrameWidth-16)<<1 &&
          js >= 0 && js <= (FrameHeight-16))
      {
        for (delta_y=-1; delta_y<=1; delta_y++)
        {
          for (delta_x=-1; delta_x<=1; delta_x++)
          {
            ...
            if (it >= 0 && it <= (FrameWidth-16)<<1 &&
                jt >= 0 && jt <= (FrameHeight-16) &&
                ib >= 0 && ib <= (FrameWidth-16)<<1 &&
                jb >= 0 && jb <= (FrameHeight-16))
            {
              /* compute prediction error */
              local_dist = bdist2(
                ref + (is>>1) + (FrameWidth<<1)*(js>>1),
                ref + FrameWidth + (it>>1) + (FrameWidth<<1)*(jt>>1),
                mb,             /* current mb location */
                FrameWidth<<1,       /* adjacent line distance */
                is&1, js&1, it&1, jt&1, /* half-pel flags */
                8);             /* block height */
              local_dist += bdist2(
                ref + FrameWidth + (is>>1) + (FrameWidth<<1)*(js>>1),
                ref + (ib>>1) + (FrameWidth<<1)*(jb>>1),
                mb + FrameWidth,     /* current mb location */
                FrameWidth<<1,       /* adjacent line distance */
  ...
  local_dist = bdist1(
    ref + (imins>>1) + (FrameWidth<<1)*(jmins>>1),
    ref + FrameWidth + (imint>>1) + (FrameWidth<<1)*(jmint>>1),
    mb,
    FrameWidth<<1,
    imins&1, jmins&1, imint&1, jmint&1,
    8);
  local_dist += bdist1(
    ref + FrameWidth + (imins>>1) + (FrameWidth<<1)*(jmins>>1),
    ref + (iminb>>1) + (FrameWidth<<1)*(jminb>>1),
    mb + FrameWidth,
    FrameWidth<<1,
