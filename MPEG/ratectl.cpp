/* ratectl.cpp, bitrate control routines (linear quantization only currently) */
...
int Xi, Xp, Xb, /*r*/ReactionParameter, d0i, d0p, d0b;
double avg_act;
static int R, T, d;
...
void rc_init_seq()
{
  ...
  if (ReactionParameter==0)  ReactionParameter = (int)floor(2.0*bit_rate/frame_rate + 0.5);
  ...
  if (d0i==0) d0i = (int)floor(10.0*ReactionParameter/31.0 + 0.5);
  if (d0p==0) d0p = (int)floor(10.0*ReactionParameter/31.0 + 0.5);
  if (d0b==0) d0b = (int)floor(1.4*10.0*ReactionParameter/31.0 + 0.5);
/*
...

{
  ...
  if (T<Tmin)
    T = (int)Tmin;		// (int) added PHD 2012/04/23
...

{
  ...
  for (j=0; j<height2; j+=16)
    ...
    {
      p = frame + ((pict_struct==BOTTOM_FIELD)?FrameWidth:0) + i + width2*j;
      ...
      {
        /* field */
        var = var_sblk(p,FrameWidth<<1);
        if (var<actj) actj = var;
        var = var_sblk(p+8,FrameWidth<<1);
        if (var<actj) actj = var;
        var = var_sblk(p+FrameWidth,FrameWidth<<1);
        if (var<actj) actj = var;
        var = var_sblk(p+FrameWidth+8,FrameWidth<<1);
        if (var<actj) actj = var;
      }
...
void rc_update_pict()
{
  ...
  {
  case I_TYPE:
    Xi = (int)X;		// (int) added PHD 2012/04/23
    d0i = d;
    break;
  case P_TYPE:
    Xp = (int)X;		// (int) added PHD 2012/04/23
    d0p = d;
    Np--;
    break;
  case B_TYPE:
    Xb = (int)X;		// (int) added PHD 2012/04/23
    d0b = d;
    Nb--;
    break;
  }
...
int rc_start_mb()
{
  ...
  if (q_scale_type)
  {
    mquant = (int) floor(2.0*d*31.0/ReactionParameter + 0.5);
  ...
  {
    mquant = (int) floor(d*31.0/ReactionParameter + 0.5);
    mquant <<= 1;
...
void calc_vbv_delay()
{
  ...
  {
    if (!quiet)
      WarningMPEG("vbv_delay overflow!\n");
  }
