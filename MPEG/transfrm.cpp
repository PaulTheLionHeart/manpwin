/* transfrm.cpp,  forward / inverse transformation                            */
    ...
    for (i=0; i<FrameWidth; i+=16)
    {
      for (n=0; n<block_count; n++)
      {
        ...
        if (cc==0)
        {
          ...
          {
            /* field DCT */
            offs = i + ((n&1)<<3) + FrameWidth*(j+((n&2)>>1));
            lx = FrameWidth<<1;
          }
          else
          {
  ...
  for (j0=0; j0<height2; j0+=16)
    for (i0=0; i0<FrameWidth; i0+=16)
    {
      ...
      else
      {
        ...
        for (j=0; j<8; j++)
        {
          offs = FrameWidth*((j<<1)+j0) + i0;
          for (i=0; i<16; i++)
          {
            blk0[16*j+i] = cur[offs] - pred[offs];
            blk1[16*j+i] = cur[offs+FrameWidth] - pred[offs+FrameWidth];
            offs++;
          }
        }
        /* correlate fields */
