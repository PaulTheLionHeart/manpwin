/* predict.cpp, motion compensated prediction                                 */
...
void predict(unsigned char *reff[],unsigned char *refb[],unsigned char *cur[3], int secondfield, struct mbinfo *mbi)
{
  ...
  for (j=0; j<height2; j+=16)
    ...
    {
      predict_mb(reff,refb,cur,FrameWidth,i,j,pict_type,pict_struct,
...
static void predict_mb(unsigned char *oldref[],unsigned char *newref[],unsigned char *cur[], int lx, int bx, 
			int by, int pict_type, int pict_struct, int mb_type, int motion_type, int secondfield, 
			int PMV[2][2][2], int mv_field_sel[2][2], int dmvector[2])
{
  ...
  if ((mb_type & MB_FORWARD) || (pict_type==P_TYPE))
  {
    ...
    if (pict_struct==FRAME_PICTURE)
    {
      ...
      else
      {
        ...
        if (!quiet)
          WarningMPEG("invalid motion_type\n");
      }
    }
 ...
 */
static void pred(unsigned char *src[], int sfield, unsigned char *dst[], int dfield, int lx, int w, 
...
static void clearblock(unsigned char *cur[], int i0, int j0)
{
  ...
  p = cur[0] + ((pict_struct==BOTTOM_FIELD) ? FrameWidth : 0) + i0 + width2*j0;
