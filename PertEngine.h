...
class CPerturbation 
    {
    public:
	int	initialiseCalculateFrame(CDib *DibIn, CSlope *Slope, int WidthIn, int HeightIn, int threshold, BigDouble xZoomPointin, BigDouble yZoomPointin, BigDouble BigWidthIn, int decimals, CTZfilter *TZfilter, HWND hwndIn, 
		int ThreadIn, double *wpixelsIn, double paramIn[], double potparamIn[], int PaletteShiftIn, int *PlotTypeIn, int SlopeTypeIn, double lightDirectionDegrees, double bumpMappingDepth, double bumpMappingStrength, 
		int PaletteStartIn, double LightHeightIn, int PertColourMethodIn, int PalOffset, double IterDiv, bool EnableApproximationIn);
	...
	int	CalculatePointExp(int x, int y, ExpComplex ExpDeltaSub0, double bailout, Point *glitchPoints, int user_data(HWND hwnd), CTZfilter *TZfilter, CTrueCol *TrueCol, HANDLE ghMutex);
	int	ReferenceZoomPoint(BigComplex& centre, int MAX_ITERATIONS, int user_data(HWND hwnd), char* StatusBarInfo, int *pPertProgress, double bailout, bool Multi);
	...
	int	iterateFractalWithPerturbationBLA(Complex *XSubN, int MaxIteration, double bailout, Complex& DeltaSub0, BLAS *Bla, CTZfilter *TZfilter, Complex *z, int user_data(HWND hwnd));
	int	iterateFractalWithPerturbationBLAExp(ExpComplex *XSubN, int MaxIteration, double bailout, ExpComplex& DeltaSub0, BLAS *Bla, CTZfilter *TZfilter, ExpComplex *z, int user_data(HWND hwnd));
	int	iterateFractalWithPerturbation(Complex *XSubN, Complex *DeltaSub0, Complex *DeltaSubN, int MaxIteration, double bailout, int x, int y, int user_data(HWND hwnd), CTZfilter *TZfilter,
		ExpComplex *ExpDC, Complex *dc, bool *glitched);
	int	iterateFractalWithPerturbationExp(ExpComplex *ExpXSubN, ExpComplex *DeltaSub0, ExpComplex *ExpDeltaSubN, int MaxIteration, double bailout, int x, int y, int user_data(HWND hwnd),
...
#define	BLA_DOUBLE		3	// use noremal double float for BLA
#define	BLA_FLOATEXP		4	// use floatexp for BLA
#define	BLA_DBL_UNSUPPORTED	5	// use double for calculation, but don't process BLA table at this stage as this fractal is unsupported by BLA
#define	BLA_EXP_UNSUPPORTED	6	// use floatexp for calculation, but don't process BLA table at this stage as this fractal is unsupported by BLA
