...
thread_local	Complex		c, z;
thread_local	DDComplex	zDD, cDD;
thread_local	QDComplex	zQD, cQD;
...
thread_local int jump_index;//, InitJumpIndex;
...
thread_local	int StoPtr, LodPtr, OpPtr;					// static CAE fp
unsigned int LastOp;						// CAE fp made non-static
std::vector<ConstArg> v_storage;
ConstArg* v = nullptr;
//struct ConstArg *v = (struct ConstArg *)0;			// was static CAE fp
void (**f)(void) = (void(far * far *)(void))0;			// static CAE fp

std::vector<unsigned short> gStoreIdx;
std::vector<unsigned short> gLoadIdx;
unsigned gStoreCap = 0;
unsigned gLoadCap = 0;
...
static	int real_count;
	...
	short int ismand = 1;
	unsigned int posp, vsp;
static	unsigned int n, NextOp, InitN;
static	int paren, ExpectingArg;
thread_local int InitLodPtr, InitStoPtr, InitOpPtr, LastInitOp;	// was static CAE fp
...
static int isconst_pair(char *Str) 
	...
	if(isdigit(Str[j]) || (Str[j] == '-' && (isdigit(Str[j+1]) || Str[j+1] == '.')) || Str[j] == '.') 
	    {
            answer = 1;
	    }
	}
...
struct ConstArg far *isconst(char *Str, int Len) 
    ...
    if(isdigit(Str[0]) || (Str[0] == '-' && (isdigit(Str[1]) || Str[1] == '.')) || Str[0] == '.') 
	{
...
static int ParseStr(char *Str, int pass) 
    ...
    jump_index = 0;
    uses_jump = 0;
    memset(jump_control, 0, sizeof(jump_control));
...
//    jump_index = 0;
    ...
    // after o is allocated/assigned
    for (int i = 0; i < (int)Max_Ops; ++i)
	{
//	o[i].op = OpCode::LEGACY;  // default: dispatch by o[i].f
...
//		    Store[StoPtr++] = Load[--LodPtr];
		    ...
		    // new parallel index table (add)
		    // preserve exact legacy ordering:
		    // 1) pre-decrement LodPtr
		    // 2) read load index at that LodPtr
		    // 3) store it, then increment StoPtr

		    uint16_t src = gLoadIdx[--LodPtr];
		    gStoreIdx[StoPtr++] = src;
		    Equals++;
		    }
		break;
	    default:
...
int	CParser::FormulaFloat(Complex *zIn, Complex *cIn)		// used for direct formula parser so it can be used with real time Julia
    {
    int	    ret;
...
int	CParser::DDFormula(DDComplex *zIn, DDComplex *cIn)	// used for direct formula parser so it can be used with real time Julia
    {
    int	    ret;
...
int	CParser::QDFormula(QDComplex *zIn, QDComplex *cIn)	// used for direct formula parser so it can be used with real time Julia
    {
    int	    ret;
...
#undef Arg2
	...
	case DOUBLEFLOAT:
	    z = vloc[3].a.d;
	    return (m_ctx.Arg1->d.x == 0.0);

	case DOUBLEDOUBLE:
	    zDD = vloc[3].a.dd;
	    return (to_double(m_ctx.Arg1->dd.x) == 0.0);
...
int CParser::FormPerPixel(void)
	...
	case QUADDOUBLE:   m_ctx.v[0].a.qd = cQD; break;
	}
...
//    CParser* g_activeParser = &parser;   // however you store it (global or TLS)
	...
	case QUADDOUBLE:   zQD = m_ctx.v[3].a.qd; break;
	}

    return m_ctx.overflow ? 0 : 1;
    }
...
int fill_if_group(int endif_index, JUMP_PTRS_ST* jump_data)
	...
	switch (jump_control[i].type) 
		...
		jump_control[i].ptrs.JumpOpPtr += 1;   // <-- IMPORTANT: land after the ELSE jump opcode
		...
		jump_control[i].ptrs.DestOpPtr = jump_data[ljp].JumpOpPtr + 1;
		jump_control[i].ptrs.DestLodPtr = jump_data[ljp].JumpLodPtr;
		jump_control[i].ptrs.DestStoPtr = jump_data[ljp].JumpStoPtr;
		...
		jump_control[i].ptrs.DestOpPtr = jump_data[endif_index].JumpOpPtr + 1;
		jump_control[i].ptrs.DestLodPtr = jump_data[endif_index].JumpLodPtr;
		jump_control[i].ptrs.DestStoPtr = jump_data[endif_index].JumpStoPtr;
...
int fill_jump_struct(void)
	...
	{
	void(*fn)(void) = f[OpPtr];   // <-- declare fn
	if(find_new_func) 
	    {
	    switch (jump_control[i].type) 
		{
	...
	else if (JumpFunc && (fn == JumpFunc ||
	    (JumpFunc == PtrStkJump && fn == StkJump) ||
	    (JumpFunc == StkJump && fn == PtrStkJump)))
//	else if(*(f[OpPtr]) == JumpFunc)
   ...
   // Following for safety only; all should always be false
    if(i != jump_index || jump_control[i - 1].type != 4 || jump_control[0].type != 1)
	{
	return 1;
	}
...
// Note - this function will be called twice to fill in the components of a complex constant. See is_complex_constant() below
...
// frmgettoken fills token structure; returns 1 on success and 0 on NOT_A_TOKEN and END_OF_FORMULA
...
int frmgettoken(FILE * openfile, struct token_st * this_token)
	...
	CASE_TERMINATOR:
	    ...
	    else if (this_token->token_str[0] == '\n' || this_token->token_str[0] == ',' || this_token->token_str[0] == ':' ) 
		{
		...
		{
		// the following function will set token_type to PARENS and token_id to OPEN_PARENS if this is not the start of a complex constant
...
int BadFormula(void) 
    ...
    //  this is called when a formula is bad, instead of calling the normal functions which will produce undefined results
    return 1;
    }
...
//	Load = (struct Arg far * far *)(Store + MAX_STORES);
	...
	// parser.cpp :: parser_allocate()

	gLoadIdx.clear();
	gStoreIdx.clear();
	gLoadIdx.resize(MAX_LOADS);
	gStoreIdx.resize(MAX_STORES);
	gStoreCap = (unsigned)gStoreIdx.size();
	gLoadCap = (unsigned)gLoadIdx.size();

	v_storage.resize(Max_Args);
	v = v_storage.data();
...
void frm_error(FILE * open_file, long begin_frm)
	...
	while(!done) 
	    ...
	    if ((tok.token_type == END_OF_FORMULA) || (tok.token_type == OPERATOR && (tok.token_id == 0 || tok.token_id == 11)) || (tok.token_type == NOT_A_TOKEN && tok.token_id == END_OF_FILE))
		{
		done = 1;
