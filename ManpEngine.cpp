...
int	CPixel::PerformWorklist(int NumberThreadsIn, int ThreadNumber, bool *ThreadComplete, HANDLE *ghMutexIn, int user_data(HWND hwnd))
    ...
    *ThreadComplete = false;
    ...
    if (RunThread(hwnd, ThreadNumber, StripWidth, &Slope, Fractal, /*&Big_xxmin, &Big_xxmax, &Big_yymin, &xxmin, &xxmax, &yymin, */user_data) < 0)
	return -1;
...
int	CPixel::RunThread(HWND hwnd, int ThreadNumberIn, int StripWidthIn, CSlope *Slope, CFract *Fractal, //BigDouble *Big_xxmin, BigDouble *Big_xxmax, BigDouble *Big_yymin, 
		/*double *xxmin, double *xxmax, double *yymin, */int user_data(HWND hwnd))
    {
    int	    i;
